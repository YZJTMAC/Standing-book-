...
/*		之前的单个筛选条件查询，现在改成多个筛选条件查询
		...
		for(ProjectSummariesEntity ps : projectSummaries){
			...
			BigDecimal incomeAmount12to15 = new BigDecimal(0);
			BigDecimal zero = new BigDecimal(0);
			for(ProjectYearSummaryEntity yearSummary : ProjectYearSummarys){
				if(yearSummary.getYear() <= 2015){
					incomeAmount12to15 = incomeAmount12to15.add(yearSummary.getIncomeamount()==null?zero:yearSummary.getIncomeamount());
				}
			}
			ps.setProjectYearSummarys(ProjectYearSummarys);
			ps.setRealIncomeAmount12To15(incomeAmount12to15);
		}
		vo.setTotalFlag(-1);
		total = projectSummariesMapper.selectSummariesByCondition(vo).get(0).getTotal();
	...
	public ProjectSummariesQueryVo setConditionToVO(QueryTermsForm form,ProjectSummariesQueryVo vo){
		...
		vo.setProjectCompany(form.getCompany());
